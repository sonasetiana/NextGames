...
class GameRepositoryImpl @Inject constructor(
    ...
    override fun getAllGames(limit: Int, page: Int): Flowable<Results<List<Game>>> {
        val results = PublishSubject.create<Results<List<Game>>>()
        remote.getAllGames(limit, page)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .take(1)
            .subscribe({ items ->
                results.onNext(Results.Success(items.results.map { DataMapper.mappingGameData(it)}))
            },{
    ...
    override fun searchGames(keyword: String): Flowable<Results<List<Game>>> {
        val results = PublishSubject.create<Results<List<Game>>>()
